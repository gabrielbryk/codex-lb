...
Before awaiting durable cleanup, the proxy MUST publish the denied id to the live session. Publication MUST be serialized with the submitter's final tombstone check and upstream send so either an already-started send finishes first or publication wins and fences that send. Immediately before dispatch, any already-prepared request carrying that id as a proxy-injected anchor MUST fail closed without sending another upstream frame, unless the request's captured pre-injection payload is known to have carried full context on its own, in which case the proxy MUST self-heal at most once per client request by dropping the anchor and reissuing that same submission with the captured full-context body instead of raising, since a native client that never set the anchor cannot drop it on its own and would otherwise retry into the same denial indefinitely. This revalidation MUST close the retirement/dispatch race; it MUST NOT reject a client-supplied anchor merely because the same id is tombstoned for proxy injection. The same self-heal MUST also apply when session creation instead fails because the anchor's owner account is unavailable (`previous_response_owner_unavailable`): given the same full-context discriminator and once-per-request bound, the proxy MUST drop the proxy-injected anchor and its owner pin and continue into ordinary account-neutral selection instead of failing closed, and MUST otherwise leave the fail-closed raise unchanged. The same self-heal MUST also apply on the WebSocket connect path's equivalent `previous_response_owner_unavailable` raises (both the preferred-account mismatch and the preferred-account-missing outcomes of account selection), reselecting account-neutral before falling through to the existing fail-closed raise. The same self-heal MUST also apply when hard session/turn-state affinity pins the owner without any `previous_response_id` anchor at all (the client's own full-resend body already proved it carries the durable turn's complete context), dropping only the account pin, except when the pinned account is required for input-file access, in which case the proxy MUST keep failing closed.
