...
2. **Kernel-enforced isolation over reviewed isolation.** `unshare --map-root-user --net` plus `ip link set lo up` works unprivileged and makes external egress impossible rather than merely unconfigured. Files created inside map back to the invoking account outside. `--no-network-namespace` exists for hosts without `unshare` and demands an explicit second flag, so the safe path is never the accidental one.
