...
- [x] 5.5 `tests/integration/test_affinity_invite_migration.py`: scope the post-downgrade drift
  assertion to the tables that suite governs. Downgrading to the affinity parent unapplies
  everything above the topmost merge, so global drift emptiness is not an invariant for any
  later column-adding revision; the two assertions at head stay strict.
