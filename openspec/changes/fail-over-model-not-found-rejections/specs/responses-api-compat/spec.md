...
When upstream rejects a Responses request before accepting the response with either account/model rejection envelope, the proxy MUST classify the failure internally as `account_model_unsupported`. The envelopes are `invalid_request_error` with the exact message `The '<model>' model is not supported when using Codex with a ChatGPT account.`, and `model_not_found` with the exact message `The model `<model>` does not exist or you do not have access to it.` Upstream returns the second envelope with HTTP 404 while the account's own model list still advertises the model. The quoted model MUST match the requested model. For native WebSocket, HTTP responses bridge, and raw
...
rejection, with its original status, instead of replacing it with `no_accounts`, `stream_incomplete`, or
...
- **THEN** the client receives the original upstream rejection with its original status (HTTP 400 `invalid_request_error` or HTTP 404 `model_not_found`)
...
- **AND** the skipped account's original rejection is not used as a fallback
...
#### Scenario: model-access 404 retries another advertising account

- **GIVEN** two accounts advertise the requested model in the current routing snapshot
- **AND** upstream rejects the first account with HTTP 404 `model_not_found` and the exact message `The model `<model>` does not exist or you do not have access to it.` before `response.created`
- **WHEN** the request has no hard account or uploaded-file binding
- **THEN** the proxy excludes the first account for this request and retries once on the second account
- **AND** it does not penalize the first account's global health

#### Scenario: model-access message for a different model does not fail over

- **WHEN** upstream returns `model_not_found` whose message quotes a model other than the requested model
- **THEN** the proxy does not classify the failure as `account_model_unsupported`
