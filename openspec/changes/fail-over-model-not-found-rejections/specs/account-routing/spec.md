...
#### Scenario: Model-access rejection leaves account health untouched

- **GIVEN** account A is selected for a model its account list advertises but upstream no longer lets it use
- **WHEN** upstream returns HTTP 404 `model_not_found` with the message `The model `<model>` does not exist or you do not have access to it.`
- **THEN** the proxy does not increment account A's transient error count and does not mark it rate-limited, quota-exceeded, or permanently failed
- **AND** the skip is logged

#### Scenario: Model-access message under another code still penalizes the account

- **GIVEN** account A is selected for a request
- **WHEN** upstream fails with a code other than `model_not_found`, or with a known status other than 404, whose message matches the model-access rejection
- **THEN** the proxy records the account-health penalty for account A as before
